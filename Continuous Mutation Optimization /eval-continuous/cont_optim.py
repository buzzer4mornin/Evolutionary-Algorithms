...
MAX_GEN = 1500  # maximum number of generations
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, mutate_ind, *, map_fn=map, log=None):
    ...
    for G in range(max_gen):
        ...
        pop = offspring[:]
        if G%40==0:
            mutate_ind.step_size = mutate_ind.step_size * 0.993
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [xover, mut], tournament_selection, mutate_ind, map_fn=map,
                                         log=log)
